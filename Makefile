...
$(BUILD_DIR)/dic.h: dic.txt $(BUILD_DIR)
	java -jar ${RESCOMP} -n dictionary -m WORDS -i dic.txt -o $(BUILD_DIR)/dic.h
...
hangout.dux: $(BUILD_DIR)/hangout.bin $(BUILD_DIR)
	java -jar ${RESCOMP} -m SIGNER -n $$(git log -1 | head -1 | sed 's/commit //' | cut -c1-8) -t hangout -d "The classic paper game" -i $(BUILD_DIR)/hangout.bin -o hangout.dux
